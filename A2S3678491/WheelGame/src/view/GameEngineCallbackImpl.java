...
public class GameEngineCallbackImpl implements GameEngineCallback
{
   ...
   public void nextSlot(Slot slot, GameEngine engine)
   {
      ...
      logger.log(Level.FINE, String.format("Next slot: %s", slot.toString()), engine);
   }
   ...
   {  
      ...
      //using StringBuffer to avoid having to create many String objects 
      StringBuffer resultOfPlayers = getResultOfAllPlayers(engine);   
      
      logger.log(Level.INFO, String.format("\n%s", resultOfPlayers));    
   }

   private StringBuffer getResultOfAllPlayers(GameEngine engine)
   {
	   StringBuffer resultOfPlayer =  new StringBuffer();
	   for(Player player : engine.getAllPlayers())
	   {
		   resultOfPlayer.append(player.toString());
	   }
